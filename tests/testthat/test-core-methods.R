...
test_that("activate_map supports plain activation and transition arguments", {
  session <- make_mock_session()
  game <- PhaserGame$new()
  game$set_shiny_session(session)
  ...
  expect_true(any(grepl(
    'activateMap("level_2", null, null, null, [], []);',
    msgs, fixed = TRUE
  )))
})
...
test_that("saved games use Phaser snapshots and server disk persistence", {
  game_js <- readLines(system.file("www", "phaser-game.js", package = "shinyphaser"), warn = FALSE)
  example <- readLines(system.file("examples", "save-load.R", package = "shinyphaser"), warn = FALSE)
  ...
  expect_true(any(grepl('game$load_game("quick-save")', example, fixed = TRUE)))
})
